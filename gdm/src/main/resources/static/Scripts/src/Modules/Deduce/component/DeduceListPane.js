...
define([
    ...
    return declare([MapFloatingPane,_WidgetsInTemplateMixin, Evented], {
        ...
        {
            var me = this;
            request.post("/deduce/getDeduceTasks", {
                data: { },
                handleAs: "json"
            }).then(function (List) {
                
                array.forEach(List.data, function (o) {
                    if (o.id) {
        ...
        EditGuardTask: function (task) {
            topic.publish('egis/Deduce/EditTask', task);
            this.close();
        },

        StartGuardTask: function (task) {
            topic.publish('egis/Deduce/StartTask', task);
            this.close();
        },


        CreateButtonClick: function () {
            topic.publish('egis/Deduce/CreateTask', { TASKID: GetTimeIDString() });
            this.close();
        }
